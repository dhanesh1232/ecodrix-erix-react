...
// ─── Block options ────────────────────────────────────────────────────────────
const BLOCK_OPTIONS = [
  { label: "Paragraph",  short: "P",   tag: "p",          kbd: "⌘⌥0", icon: <Type     size={12} /> },
  { label: "Heading 1",  short: "H1",  tag: "h1",         kbd: "⌘⌥1", icon: <Heading1 size={12} /> },
  { label: "Heading 2",  short: "H2",  tag: "h2",         kbd: "⌘⌥2", icon: <Heading2 size={12} /> },
  { label: "Heading 3",  short: "H3",  tag: "h3",         kbd: "⌘⌥3", icon: <Heading3 size={12} /> },
  { label: "Heading 4",  short: "H4",  tag: "h4",         kbd: "⌘⌥4", icon: <Heading4 size={12} /> },
  { label: "Heading 5",  short: "H5",  tag: "h5",         kbd: "⌘⌥5", icon: <Heading5 size={12} /> },
  { label: "Heading 6",  short: "H6",  tag: "h6",         kbd: "⌘⌥6", icon: <Heading6 size={12} /> },
  { label: "Quote",      short: "Q",   tag: "blockquote", kbd: "⌘⌥Q", icon: <Quote    size={12} /> },
  { label: "Code Block", short: "{ }", tag: "pre",        kbd: "⌘⌥C", icon: <Code2    size={12} /> },
] as const;
        ...
        {BLOCK_OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn("erix-flex-1 erix-text-[12px]", isActive && "erix-font-semibold")}>
                {opt.label}
              </span>
              <DropdownMenuShortcut className="erix-text-[10px] erix-opacity-30">
                {opt.kbd}
              </DropdownMenuShortcut>
              {isActive && <Check className="erix-h-3 erix-w-3 erix-text-primary erix-shrink-0" />}
            </DropdownMenuItem>
          );
        })}
      </DropdownMenuContent>
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        ...
        >
          <span className="erix-truncate erix-leading-none">{active?.abbr ?? "−"}</span>
          <ChevronDown
            size={9}
            className={cn(
      ...
      <DropdownMenuContent
        ...
        sideOffset={5}
        onCloseAutoFocus={(e) => e.preventDefault()}
        className={cn("erix-min-w-[130px] erix-p-1", popoverRadius, shadowClass)}
      >
        {options.map((opt) => {
...
const SubPopoverContent: React.FC<{ children: React.ReactNode; align?: "start" | "end" | "center" }> = ({
  children,
  align = "start",
}) => (
  <PopoverContent
...
const ListsGroup: React.FC<{ engine: any; ctx: any }> = ({ engine, ctx }) => (
  <Popover>
    <PopoverTrigger asChild>
      <ToolbarBtn tooltip="Lists" active={ctx.isList || ctx.isOrderedList || ctx.isTaskList}>
        <List size={14} />
      </ToolbarBtn>
    </PopoverTrigger>
    <SubPopoverContent>
      <ToolbarBtn tooltip="Bullet list"   active={ctx.isList && !ctx.isOrderedList && !ctx.isTaskList} onClick={() => engine?.bulletList()}><List size={14} /></ToolbarBtn>
      <ToolbarBtn tooltip="Numbered list" active={ctx.isOrderedList}  onClick={() => engine?.orderedList()}><ListOrdered size={14} /></ToolbarBtn>
      <ToolbarBtn tooltip="Task list"     active={ctx.isTaskList}     onClick={() => engine?.taskList()}><ListTodo size={14} /></ToolbarBtn>
    </SubPopoverContent>
  </Popover>
);
...
const AlignGroup: React.FC<{ engine: any; ctx: any }> = ({ engine, ctx }) => {
  const Icon =
    ctx.textAlign === "center"  ? AlignCenter  :
    ctx.textAlign === "right"   ? AlignRight   :
    ctx.textAlign === "justify" ? AlignJustify :
    AlignLeft;

  return (
    <Popover>
      <PopoverTrigger asChild>
        <ToolbarBtn tooltip="Alignment" active={!!ctx.textAlign && ctx.textAlign !== "left"}>
          <Icon size={14} />
        </ToolbarBtn>
      </PopoverTrigger>
      <SubPopoverContent>
        <ToolbarBtn tooltip="Left"    active={!ctx.textAlign || ctx.textAlign === "left"}    onClick={() => engine?.align("left")}><AlignLeft size={14} /></ToolbarBtn>
        <ToolbarBtn tooltip="Center"  active={ctx.textAlign === "center"}                   onClick={() => engine?.align("center")}><AlignCenter size={14} /></ToolbarBtn>
        <ToolbarBtn tooltip="Right"   active={ctx.textAlign === "right"}                    onClick={() => engine?.align("right")}><AlignRight size={14} /></ToolbarBtn>
        <ToolbarBtn tooltip="Justify" active={ctx.textAlign === "justify"}                  onClick={() => engine?.align("justify")}><AlignJustify size={14} /></ToolbarBtn>
      </SubPopoverContent>
    </Popover>
  );
};
...
const IndentGroup: React.FC<{ engine: any }> = ({ engine }) => (
  <Popover>
    <PopoverTrigger asChild>
      <ToolbarBtn tooltip="Indent / Outdent"><Indent size={14} /></ToolbarBtn>
    </PopoverTrigger>
    <SubPopoverContent>
      <ToolbarBtn tooltip="Outdent" onClick={() => engine?.outdent()}><Outdent size={14} /></ToolbarBtn>
      <ToolbarBtn tooltip="Indent"  onClick={() => engine?.indent()}><Indent size={14} /></ToolbarBtn>
    </SubPopoverContent>
  </Popover>
);
...
const INSERT_ITEMS = [
  ...
  { label: "2 Columns", icon: <Columns2           size={13} />, fn: (e: any) => e?.columns(2) },
  { label: "3 Columns", icon: <Columns3           size={13} />, fn: (e: any) => e?.columns(3) },
  { label: "Callout",   icon: <Info               size={13} />, fn: (e: any) => e?.callout() },
  { label: "Toggle",    icon: <ChevronRightSquare size={13} />, fn: (e: any) => e?.toggle() },
  { label: "Divider",   icon: <Minus              size={13} />, fn: (e: any) => e?.horizontalRule(), sep: true },
];
...
const InsertDropdown: React.FC<{ engine: any }> = ({ engine }) => {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <ToolbarBtn tooltip="Insert block"><Plus size={14} /></ToolbarBtn>
      </PopoverTrigger>
      <PopoverContent
        side="bottom"
        align="end"
...
// ─── Overflow "…" button ──────────────────────────────────────────────────────
interface OverflowSection { id: string; label: string; node: React.ReactNode }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <TooltipProvider delayDuration={600}>
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className={cn(
                  ...
                  open && "erix-bg-accent erix-text-foreground erix-border-border/50",
            ...
            <TooltipContent side="bottom" sideOffset={6} className="erix-text-[11px]">
              More
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      ...
      >
        {sections.map((s, i) => (
          <div key={s.id}>
            {i > 0 && <div className="erix-h-px erix-bg-border/60 erix-mb-1.5" />}
...
> = (rawProps) => {
  ...
  const { containerRadius, shadowClass, popoverRadius, buttonRadius } = useErixStyle();
  ...
  const sections = React.useMemo<Section[]>(() => {
    ...
    if (cfg.history) {
      s.push({
        ...
        node: (
          <ToolbarGroup>
            <ToolbarBtn tooltip="Undo  ⌘Z"  disabled={!ctx.canUndo} onClick={() => engine?.undo()}><Undo2 size={14} /></ToolbarBtn>
            <ToolbarBtn tooltip="Redo  ⌘⇧Z" disabled={!ctx.canRedo} onClick={() => engine?.redo()}><Redo2 size={14} /></ToolbarBtn>
          </ToolbarGroup>
        ),
      });
    }
    ...
    if (cfg.textFormat) {
      s.push({
        ...
        node: (
          <ToolbarGroup>
            <ToolbarBtn tooltip="Bold  ⌘B"      active={ctx.bold}      onClick={() => engine?.bold()}><Bold size={14} /></ToolbarBtn>
            <ToolbarBtn tooltip="Italic  ⌘I"    active={ctx.italic}    onClick={() => engine?.italic()}><Italic size={14} /></ToolbarBtn>
            <ToolbarBtn tooltip="Underline  ⌘U" active={ctx.underline} onClick={() => engine?.underline()}><Underline size={14} /></ToolbarBtn>
            <ToolbarBtn tooltip="Strikethrough" active={ctx.strike}    onClick={() => engine?.strike()}><Strikethrough size={14} /></ToolbarBtn>
            <ToolbarBtn tooltip="Inline code"   active={ctx.code}      onClick={() => engine?.inlineCode()}><Code size={14} /></ToolbarBtn>
            <ToolbarBtn tooltip="Clear formatting"                      onClick={() => engine?.clearFormatting()}><Eraser size={14} /></ToolbarBtn>
          </ToolbarGroup>
        ),
      });
    }
    ...
    if (cfg.colors) {
      s.push({
        ...
        node: (
          <ColorPickerNative>
            <ToolbarBtn tooltip="Text & highlight color" active={!!(ctx.foreColor || ctx.backColor)}>
              <Palette size={14} />
            </ToolbarBtn>
          </ColorPickerNative>
        ),
    ...
    if (cfg.lists) {
      s.push({ id: "lists",     label: "Lists",     hasSep: true, node: <ListsGroup engine={engine} ctx={ctx} /> });
    }
    if (cfg.alignment) {
      s.push({ id: "alignment", label: "Alignment", hasSep: true, node: <AlignGroup engine={engine} ctx={ctx} /> });
    }
    if (cfg.indent) {
      s.push({
        id: "indent",
      ...
      s.push({
        ...
        node: (
          <ToolbarGroup>
            {cfg.link && (
              <LinkInsertNative>
                <ToolbarBtn
                  ...
                  active={ctx.link}
                  disabled={(!ctx.hasSelection && !ctx.link) || ctx.isEmbed || ctx.isNonEditable}
                >
                  <Link2 size={14} />
                </ToolbarBtn>
              </LinkInsertNative>
            ...
            {cfg.image && (
              <ImagePickerNative>
                <ToolbarBtn tooltip="Insert image"><ImagePlus size={14} /></ToolbarBtn>
              </ImagePickerNative>
            )}
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    cfg.history, cfg.headings, cfg.fonts, cfg.textFormat, cfg.colors,
    cfg.lists, cfg.alignment, cfg.indent, cfg.link, cfg.image, cfg.table, cfg.ai,
    blockLabel, blockShort, blockTag,
    ctx.canUndo, ctx.canRedo,
    ctx.bold, ctx.italic, ctx.underline, ctx.strike, ctx.code,
    ctx.isList, ctx.isOrderedList, ctx.isTaskList,
    ctx.textAlign, ctx.foreColor, ctx.backColor,
    ctx.link, ctx.hasSelection, ctx.isEmbed, ctx.isNonEditable,
    ctx.fontFamily, ctx.fontSize,
  ]);
  ...
  React.useLayoutEffect(() => { measure(); }, [sections.length, measure]);
      ...
      >
        ...
        {isOverflowing && (
          <>
            <Sep />
            <OverflowBtn
              sections={overflow.map((s) => ({ id: s.id, label: s.label, node: s.node }))}
