...
export function ErixDevtools() {
  ...
  const [tab, setTab] = React.useState<
    "router" | "events" | "permissions" | "realtime"
  >("router");
  ...
  useErixEvent("wa.message.received", (p) =>
    appendLog("wa.message.received", p),
  );
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          >
            {(["router", "events", "permissions", "realtime"] as const).map(
              (t) => (
                <button
                  key={t}
                  type="button"
          ...
          <div style={{ padding: 12, overflowY: "auto", flex: 1 }}>
            ...
            {tab === "permissions" && (
              <div>
                ...
                {[...permissions].map((p) => (
                  <div key={p} style={{ color: "#86efac", padding: "1px 0" }}>
                    ✓ {p}
                  </div>
                ))}
              </div>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: "erix-flex", gap: 8, padding: "2px 0" }}>
